defmodule Core.Xandra do
  @moduledoc false

  def await_connected(conn, tries \\ 4) do
    try do
      Xandra.execute(conn, "SELECT now() FROM system.local;", [], pool: Xandra.Cluster)
    rescue
      Xandra.ConnectionError ->
        if tries > 0 do
          Process.sleep(50)
          await_connected(conn, tries - 1)
        else
